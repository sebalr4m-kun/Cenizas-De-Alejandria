...
-- Optimización: Soporte RBAC Modular Integrado y Lógica Unilateral de Préstamos
-- =============================================================================
...
-- Tabla Cabecera de Préstamos (Conjunto de Solicitudes)
CREATE TABLE prestamos (
    id_prestamo INT AUTO_INCREMENT PRIMARY KEY,
    id_usuario INT,
    fecha_prestamo TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    fecha_devolucion_real TIMESTAMP NULL DEFAULT NULL,
    estado_prestamo VARCHAR(20) DEFAULT 'ACTIVO', -- 'ACTIVO' o 'DEVUELTO'
    FOREIGN KEY (id_usuario) REFERENCES usuarios(id_usuario) ON DELETE SET NULL
);

-- Tabla Detalle de Préstamos (Ítems incluidos en el conjunto)
CREATE TABLE detalle_prestamo (
    id_detalle INT AUTO_INCREMENT PRIMARY KEY,
    id_prestamo INT,
    id_insumo INT,
    estado_item VARCHAR(20) DEFAULT 'PRESTADO', -- 'PRESTADO' o 'DEVUELTO'
...
-- Disparador 1: Al insertar un ítem prestado, pasa el insumo a estado 'PRESTADO'
CREATE TRIGGER trg_detalle_prestamo_nuevo
AFTER INSERT ON detalle_prestamo
FOR EACH ROW
BEGIN
    IF NEW.estado_item = 'PRESTADO' THEN
        UPDATE insumos
        SET estado = 'PRESTADO'
...
-- Disparador 2: Al devolver un ítem individual, el insumo vuelve a estar 'DISPONIBLE'
CREATE TRIGGER trg_detalle_devolucion
AFTER UPDATE ON detalle_prestamo
FOR EACH ROW
BEGIN
    IF NEW.estado_item = 'DEVUELTO' AND OLD.estado_item != 'DEVUELTO' THEN
        UPDATE insumos
